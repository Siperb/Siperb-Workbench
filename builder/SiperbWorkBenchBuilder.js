...
const SiperbWorkBenchBuilder = {
     ...
     */
    LoadBrowserPhone(iframeElement, options){
        options = options || {};
        return new Promise(async function(resolve, reject){
            // Load the Browser Phone into the IFRAME
            ...
            // Resolve tree URL: options > window.TestConfig > localStorage DEV_MODE fallback
            let treeUrl = options.treeUrl
                || (typeof window !== 'undefined' && window.TestConfig && window.TestConfig.phoneVersionTreeUrl)
                || (typeof window !== 'undefined' && typeof window.localStorage !== "undefined" && localStorage.getItem("DEV_MODE") == "yes"
                    ? "https://vgf6bcf26w6wime5enpyflps640yhqmb.lambda-url.eu-west-1.on.aws/"
                    : "https://y3amulnpc5icrmmnmvtfn44zba0ckjuq.lambda-url.eu-west-1.on.aws/");
            console.log(`LoadBrowserPhone: %cUsing tree URL: ${treeUrl}`, "color: blue;");
            let phoneVersionTree = await SiperbWorkBenchBuilder.LoadVersionTree(treeUrl, "BROWSER_PHONE_VERSION_TREE");
            if (!phoneVersionTree) {
            ...
            // Apply UseLocalFiles overrides (options > window.TestConfig)
            const localFiles = options.UseLocalFiles
                || (typeof window !== 'undefined' && window.TestConfig && window.TestConfig.UseLocalFiles)
                || [];
            phoneVersionTree = SiperbWorkBenchBuilder.ApplyLocalOverrides(phoneVersionTree, localFiles);
            // Update versionTree to set all the Loaded properties to false
            phoneVersionTree.forEach(function(asset) {
                asset.Loaded = false;
            });
            // Load the HTML file
            await SiperbWorkBenchBuilder.LoadHtml(phoneVersionTree, "web", phoneDoc);
            // Load the CSS files
            await SiperbWorkBenchBuilder.LoadCss(phoneVersionTree, "web", phoneDoc);
            // Load the JS files, and star the phone
            await SiperbWorkBenchBuilder.LoadScripts(phoneVersionTree, "web", phoneDoc);
    ...
    // Apply UseLocalFiles overrides to a version tree, replacing CDN URLs with local server paths
    ApplyLocalOverrides(versionTree, useLocalFiles) {
        if (!Array.isArray(useLocalFiles) || !useLocalFiles.length) return versionTree;
        // Normalize paths by stripping leading "./" or "/" for comparison
        function normPath(p) { return p ? p.replace(/^\.?\//, '') : ''; }
        return versionTree.map(function(asset) {
            const override = useLocalFiles.find(function(f) {
                // Exact Name match
                if (f.lib === asset.Name) return true;
                // Path match: normalize both (strip leading "./" or "/") and compare
                if (asset.File && normPath(asset.File) === normPath(f.lib)) return true;
                return false;
            });
            if (override) {
                // Use /local-files/{lib} — served by the local HTTP server (run-tests.js / serve.js)
                // This avoids file:// CORS issues and passes the builder's URL check (starts with "/")
                const url = '/local-files/' + encodeURIComponent(override.lib);
                console.log(`ApplyLocalOverrides: %cOverriding ${asset.Name || asset.File} → ${url}`, "color: orange;");
    ...
    ProvisionPhone(options){
        return new Promise(async function(resolve, reject){
            ...
            phone.PROFILE_USER_ID = provisioning.ProfileUserId;
...
export default SiperbWorkBenchBuilder;

// Attach to window for browser global usage
if (typeof window !== 'undefined') {
    window.SiperbWorkBenchBuilder = window.SiperbWorkBenchBuilder || {};
    Object.assign(window.SiperbWorkBenchBuilder, SiperbWorkBenchBuilder);
}